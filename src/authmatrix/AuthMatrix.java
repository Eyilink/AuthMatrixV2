...
public class AuthMatrix implements BurpExtension {
    ...
    // ── Match rules ───────────────────────────────────────────────────────────
    // Global default: comma-separated status codes and/or regexes applied to
    // the full response (status line + headers + body).
    // e.g.  "200, 201, 30[12]"  or  "2\d\d"  or  "200, admin"
    // Per-request override stored in requestModel column 5 ("Match Override").
    private String globalMatchRule = "2\\d\\d, 3\\d\\d";   // default = 2xx or 3xx
    ...
    private void buildUI() {
        ...
        matchLabel.setToolTipText("Comma-separated HTTP status codes or regexes matched against the full response.\nIf matched → response counts as 'access granted'.\nExample: 200, 201, 30[12]   or   2\\d\\d, admin");
        JTextField matchField = new JTextField(globalMatchRule, 18);
        matchField.setMaximumSize(matchField.getPreferredSize());
        matchField.setToolTipText(matchLabel.getToolTipText());
        matchField.setFont(new Font(Font.MONOSPACED, Font.PLAIN, 12));
        matchField.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(new Color(0x56, 0x9C, 0xD6), 1),
                BorderFactory.createEmptyBorder(1, 4, 1, 4)));
        // Update globalMatchRule on change
        matchField.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            public void insertUpdate(javax.swing.event.DocumentEvent e)  { globalMatchRule = matchField.getText().trim(); }
            public void removeUpdate(javax.swing.event.DocumentEvent e)  { globalMatchRule = matchField.getText().trim(); }
            public void changedUpdate(javax.swing.event.DocumentEvent e) { globalMatchRule = matchField.getText().trim(); }
        });
        bar.add(matchLabel);
        bar.add(matchField);
    ...
    private JPanel buildRequestPanel() {
        ...
        requestModel = new DefaultTableModel(
                new String[]{"#", "Path", "Expected Users", "Expected Roles", "Description", "Match Override"}, 0) {
        ...
        tbl.getTableHeader().setToolTipText(
                ...
                "Match Override: leave empty to use the global Match rule from the toolbar.");
        ...
        tbl.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(
                ...
                // Conflict: both Expected Users AND Expected Roles set
    ...
    private JPanel buildResultPanel() {
        ...
        p.setBorder(BorderFactory.createTitledBorder(
                "Results   ✅ Expected / Normal   🔴 Unexpected Access (bypass!)   – Not tested"));
    ...
    private void runTests() {
        ...
        new Thread(() -> {
            for (int ri = 0; ri < requests.size(); ri++) {
                ...
                String matchOverride = str(requestModel.getValueAt(ri, 5));
                final int riF = ri;
                ...
                for (int ui = 0; ui < users.size(); ui++) {
                    ...
                    String  activeRule   = matchOverride.isEmpty() ? globalMatchRule : matchOverride;
                    boolean gotAccess    = matchesAccessRule(activeRule, statusStr, rawResponse);

                    // For bypass detection we also treat any 2xx/3xx as "got access"
                    // even if the match rule didn't match (e.g. rule=300 but got 200).
                    boolean gotAccessReal = gotAccess
                            || statusStr.startsWith("2")
                            || statusStr.startsWith("3");

                    // 🔴 only: user NOT expected AND server gave access (real access)
                    // ✅ everything else
                    String cell = (!shouldAccess && gotAccessReal)
                            ? "🔴 " + statusStr
                            : "✅ " + statusStr;
    ...
    /**
     * Returns true if the response is considered "access granted" according
     * to the active match rule.
     *
     * The rule is a comma-separated list of tokens. Each token is tried as:
     *   1. An exact HTTP status code string  (e.g. "200", "404")
     *   2. A Java regex matched against the full raw response
     *      (status line + headers + body), case-insensitive.
     *
     * Examples:
     *   "200, 201"          → matches status 200 or 201
     *   "2\\d\\d"           → matches any 2xx status code
     *   "2\\d\\d, 3\\d\\d"  → default: any 2xx or 3xx
     *   "200, \"admin\""    → matches 200 OR if body contains "admin"
     *   "(?!403)"           → negative lookahead (not 403) — advanced
     */
    private static boolean matchesAccessRule(String rule, String statusCode, String rawResponse) {
        ...
        for (String token : rule.split(",")) {
            ...
            try {
                // First: try to match against the status code using full-string regex
                // (String.matches is anchored — "2\d\d" matches "200", "300" matches only "300")
                if (statusCode.matches(t)) return true;

                // Second: if the token looks like a pure status-code pattern
                // (only digits and regex digit meta-chars), do NOT search the body —
                // that prevents "300" from accidentally matching "Content-Length: 3001"
                boolean looksLikeStatusPattern = t.matches("[0-9\\\\dDwWsS.*+?|()\\[\\]^${}]+");
                if (!looksLikeStatusPattern && rawResponse != null) {
                    if (java.util.regex.Pattern.compile(t,
                            java.util.regex.Pattern.CASE_INSENSITIVE |
                            java.util.regex.Pattern.DOTALL)
                        .matcher(rawResponse).find()) return true;
                }
            } catch (java.util.regex.PatternSyntaxException ex) {
                // Invalid regex → exact string match on status code only
                if (statusCode.equals(t)) return true;
            }
        }
        return false;
    }
    ...
    private void saveSession() {
        ...
        sb.append("{\n  \"version\": 1,\n");
        sb.append("  \"globalMatchRule\":").append(jStr(globalMatchRule)).append(",\n");
        ...
        for (int ri = 0; ri < requests.size(); ri++) {
            ...
            sb.append("    {\n")
              ...
              .append("      \"matchOverride\":").append(jStr(matchOverride)).append(",\n")
    ...
    private void parseAndLoadSession(String json) throws Exception {
        ...
        // Restore global match rule if present
        String savedRule = jsonStr(json, "globalMatchRule");
        if (!savedRule.isEmpty()) globalMatchRule = savedRule;
        ...
        for (String obj : jsonObjects(requestsBlock)) {
            ...
            String matchOverride = jsonStr(obj, "matchOverride");
            ...
            requestModel.addRow(new Object[]{idx, dp, expUsers, expRoles, desc, matchOverride});
    ...
    private void showHelp() {
        JOptionPane.showMessageDialog(null,
                ...
                "   • Match Override : per-request match rule (overrides toolbar)\n\n" +
                "── Match Rules (toolbar) ──────────────────────────\n" +
                "   Defines what counts as 'access granted'.\n" +
                "   Comma-separated status codes or regexes tested against\n" +
                "   the full response (status line + headers + body):\n" +
                "     2\\d\\d, 3\\d\\d    → default (any 2xx or 3xx)\n" +
                "     200, 201        → only these status codes\n" +
                "     200, token      → 200 OR body contains 'token'\n" +
                "     (?!403)         → negative lookahead (not 403)\n" +
                "   Use 'Match Override' column for per-request rules.\n\n" +
                "── Results ────────────────────────────────────────\n" +
                "   ✅  Normal (access expected, or denial expected)\n" +
                "   🔴  Auth bypass — user NOT in Expected Users/Roles\n" +
                "       but response matched the Match rule\n" +
                "   Click any coloured cell → full Request + Response.\n\n" +
                "── Save / Load ─────────────────────────────────────\n" +
